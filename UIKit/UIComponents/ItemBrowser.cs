...
namespace HEROsMod.UIKit.UIComponents
{
	internal class ItemBrowser : UIWindow
	{
		...
		public ItemBrowser()
		{
			...
			_expandTexture = ModContent.Request<Texture2D>("HEROsMod/Images/ExpandIcon", AssetRequestMode.ImmediateLoad).Value;
			_collapseTexture = ModContent.Request<Texture2D>("HEROsMod/Images/CollapseIcon", AssetRequestMode.ImmediateLoad).Value;
			_spacerTexture = ModContent.Request<Texture2D>("HEROsMod/Images/spacer", AssetRequestMode.ImmediateLoad).Value;
			...
			_bClose = new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/closeButton", AssetRequestMode.ImmediateLoad).Value);
		...
		private void PopulateSortView()
		{
			if(DefaultSorts == null)
			{
				...
				{
					new Sort( new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortItemID", AssetRequestMode.ImmediateLoad).Value){Tooltip = HeroText("SortName.ItemID")} , (x,y)=>x.type.CompareTo(y.type)),
					new Sort( new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortValue", AssetRequestMode.ImmediateLoad).Value){Tooltip = HeroText("SortName.Value")} , (x,y)=>x.value.CompareTo(y.value)),
					new Sort( new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortAZ", AssetRequestMode.ImmediateLoad).Value){Tooltip = HeroText("SortName.Alphabetical")} , (x,y)=>x.Name.CompareTo(y.Name)),
				};
			}
		...
		private void PopulateFilterView()
		{
			if (Filters == null)
			{
				Filters = new Filter[]
				{
					new Filter( new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/filterMod", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("FilterName.ModFilter")}, x=>x.ModItem != null),
				};
			}
			//Category[] categories = Categories;
			//if (SelectedCategory != null)
			...
			Categories = new Category[] {
				new Category("Weapons"/*, x=>x.damage>0*/) {
					SubCategories = new List<Category>() {
						...
						{
							Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortAmmo", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.UseAmmoType")}, (x,y)=>x.useAmmo.CompareTo(y.useAmmo)), }
						},
					...
					Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortDamage", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.Damage")}, (x,y)=>x.damage.CompareTo(y.damage)), }
				},
				new Category("Tools"/*,x=>x.pick>0||x.axe>0||x.hammer>0*/) {
					SubCategories = new List<Category>() {
						new Category("Pickaxes", x=>x.pick>0) { Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortPick", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.PickPower")}, (x,y)=>x.pick.CompareTo(y.pick)), } },
						new Category("Axes", x=>x.axe>0){ Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortAxe", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.AxePower")}, (x,y)=>x.axe.CompareTo(y.axe)), } },
						new Category("Hammers", x=>x.hammer>0){ Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortHammer", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.HammerPower")}, (x,y)=>x.hammer.CompareTo(y.hammer)), } },
					},
				},
				new Category("Armor"/*,  x=>x.headSlot!=-1||x.bodySlot!=-1||x.legSlot!=-1*/) {
					...
					Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortDefense", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.Defense")}, (x,y)=>x.defense.CompareTo(y.defense)), }
				},
				...
				{
					Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortAmmo", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.AmmoType")}, (x,y)=>x.ammo.CompareTo(y.ammo)), }
				},
				...
				new Category("BossSummons", x=>ItemID.Sets.SortingPriorityBossSpawns[x.type] != -1 && x.type != ItemID.LifeCrystal && x.type != ItemID.ManaCrystal && x.type != ItemID.CellPhone && x.type != ItemID.IceMirror && x.type != ItemID.MagicMirror && x.type != ItemID.LifeFruit && x.netID != ItemID.TreasureMap || x.netID == ItemID.PirateMap) { // vanilla bug.
					Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortDamage", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.ProgressionOrder")}, (x,y)=>ItemID.Sets.SortingPriorityBossSpawns[x.type].CompareTo(ItemID.Sets.SortingPriorityBossSpawns[y.type])), }
				},
				new Category("Consumables", x=>x.consumable),
				new Category("Fishing"/*, x=> x.fishingPole > 0 || x.bait>0|| x.questItem*/){
					SubCategories = new List<Category>() {
						new Category("Poles", x=>x.fishingPole > 0) {Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortFish", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.PolePower")}, (x,y)=>x.fishingPole.CompareTo(y.fishingPole)), } },
						new Category("Bait", x=>x.bait>0) {Sorts = new Sort[] { new Sort(new UIImage(ModContent.Request<Texture2D>("HEROsMod/Images/sortBait", AssetRequestMode.ImmediateLoad).Value) {Tooltip = HeroText("SortName.BaitPower")}, (x,y)=>x.bait.CompareTo(y.bait)), } },
