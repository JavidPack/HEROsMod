...
namespace HEROsMod.UIKit.UIComponents
{
	internal class ItemBrowser : UIWindow
	{
		...
		public ItemBrowser()
		{
			...
			_expandTexture = HEROsMod.instance.GetTexture("Images/ExpandIcon").Value;
			_collapseTexture = HEROsMod.instance.GetTexture("Images/CollapseIcon").Value;
			_spacerTexture = HEROsMod.instance.GetTexture("Images/spacer").Value;
			...
			_bClose = new UIImage(HEROsMod.instance.GetTexture("Images/closeButton").Value);
		...
		private void PopulateSortView()
		{
			if(DefaultSorts == null)
			{
				...
				{
					new Sort( new UIImage(HEROsMod.instance.GetTexture("Images/sortItemID").Value){Tooltip = HeroText("SortName.ItemID")} , (x,y)=>x.type.CompareTo(y.type)),
					new Sort( new UIImage(HEROsMod.instance.GetTexture("Images/sortValue").Value){Tooltip = HeroText("SortName.Value")} , (x,y)=>x.value.CompareTo(y.value)),
					new Sort( new UIImage(HEROsMod.instance.GetTexture("Images/sortAZ").Value){Tooltip = HeroText("SortName.Alphabetical")} , (x,y)=>x.Name.CompareTo(y.Name)),
				};
			}
		...
		public static void ParseList2()
		{
			...
			Categories = new Category[] {
				...
				new Category("Tools"/*,x=>x.pick>0||x.axe>0||x.hammer>0*/) {
					SubCategories = new List<Category>() {
						new Category("Pickaxes", x=>x.pick>0) { Sorts = new Sort[] { new Sort(new UIImage(HEROsMod.instance.GetTexture("Images/sortPick").Value) {Tooltip = HeroText("SortName.PickPower")}, (x,y)=>x.pick.CompareTo(y.pick)), } },
						new Category("Axes", x=>x.axe>0){ Sorts = new Sort[] { new Sort(new UIImage(HEROsMod.instance.GetTexture("Images/sortAxe").Value) {Tooltip = HeroText("SortName.AxePower")}, (x,y)=>x.axe.CompareTo(y.axe)), } },
						new Category("Hammers", x=>x.hammer>0){ Sorts = new Sort[] { new Sort(new UIImage(HEROsMod.instance.GetTexture("Images/sortHammer").Value) {Tooltip = HeroText("SortName.HammerPower")}, (x,y)=>x.hammer.CompareTo(y.hammer)), } },
					},
				},
				new Category("Armor"/*,  x=>x.headSlot!=-1||x.bodySlot!=-1||x.legSlot!=-1*/) {
					...
					Sorts = new Sort[] { new Sort(new UIImage(HEROsMod.instance.GetTexture("Images/sortDefense").Value) {Tooltip = HeroText("SortName.Defense")}, (x,y)=>x.defense.CompareTo(y.defense)), }
				},
				...
				new Category("BossSummons", x=>ItemID.Sets.SortingPriorityBossSpawns[x.type] != -1 && x.type != ItemID.LifeCrystal && x.type != ItemID.ManaCrystal && x.type != ItemID.CellPhone && x.type != ItemID.IceMirror && x.type != ItemID.MagicMirror && x.type != ItemID.LifeFruit && x.netID != ItemID.TreasureMap || x.netID == ItemID.PirateMap) { // vanilla bug.
					Sorts = new Sort[] { new Sort(new UIImage(HEROsMod.instance.GetTexture("Images/sortDamage").Value) {Tooltip = HeroText("SortName.ProgressionOrder")}, (x,y)=>ItemID.Sets.SortingPriorityBossSpawns[x.type].CompareTo(ItemID.Sets.SortingPriorityBossSpawns[y.type])), }
				},
				new Category("Consumables", x=>x.consumable),
				new Category("Fishing"/*, x=> x.fishingPole > 0 || x.bait>0|| x.questItem*/){
					SubCategories = new List<Category>() {
						new Category("Poles", x=>x.fishingPole > 0) {Sorts = new Sort[] { new Sort(new UIImage(HEROsMod.instance.GetTexture("Images/sortFish").Value) {Tooltip = HeroText("SortName.PolePower")}, (x,y)=>x.fishingPole.CompareTo(y.fishingPole)), } },
						new Category("Bait", x=>x.bait>0) {Sorts = new Sort[] { new Sort(new UIImage(HEROsMod.instance.GetTexture("Images/sortBait").Value) {Tooltip = HeroText("SortName.BaitPower")}, (x,y)=>x.bait.CompareTo(y.bait)), } },
